...
| Schema | Role |
|---|---|
| `rics_mirror` | Imported RICS source mirror. Read-only to app code. Rebuilt by `pnpm sync:rics`. No app-owned FKs should point into it. |
| `public` | Storefront baseline tables that predate the app/module split. Preserved across `rics_mirror` reloads. |
| `app` | App-owned additive tables: SKU drafts/lifecycle, attributes, product family, overlays, batch operations, etc. |
| `platform` | Cross-cutting admin spine: ETL runs now; future audit, notification, feature flag, scheduled task surfaces. |
...
Request handlers consume adapters that read from `rics_mirror`. Every adapter may cache on top, but the source remains Postgres, not MDB files.
...
Overlay writes affect Zack's Retail views only. They do **not** flow back to RICS. Warehouse/POS users in RICS see only RICS data until cutover.
