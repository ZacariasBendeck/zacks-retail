...
| `legacy/inventory_quantities.csv` + `legacy/size_types.csv` + `app.sku` | `import:app-stock-from-artifact` normalizes the inventory grid into current per-store/per-SKU/per-cell stock rows | `app.stock_level` |
