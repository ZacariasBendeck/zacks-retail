...
> ⚠️ May be stale per 2026-04-26 `/index-knowledge` pass: the three sales-reporting bullets below were the original v1 contract split. Live RICS parity checks on April 25-26, 2026 showed the implemented inquiry does **not** use one authority for every sales surface. The sales roll-up strip, All Stores Summary, All Stores - 1 Row totals for single-row size types, and the `[Info]` popup are sourced from the imported inquiry-history tables in `app.inventory_history_snapshot` and `app.inventory_history_month`, while the size-grid sales modes and `Column Only` sales rows are sourced from the owned `app.inventory_sales_cell` projection imported from `inventory_quantities.csv`. See [`docs/dev/specs/2026-04-26-inventory-inquiry-sales-authority.md`](../../dev/specs/2026-04-26-inventory-inquiry-sales-authority.md).
...
- Direct navigation to `/products/inquiry/:skuCode` stays the full-page inquiry route for the menu, bookmarks, and modifier-click/new-tab workflows. SKU calls from other pages should use the app-wide `InquiryPopupProvider` / `SkuLink` path so Inventory Inquiry opens as a draggable popup over the caller's page.
...
- The MTD / STD / YTD / L/Y size-grid modes, the backend-computed `Total` row appended to the MTD / STD / YTD / L/Y tabs, the grid-level totals shown beside the `On Hand`, `Short Quantities`, `Month-to-Date Sales`, `Season-to-Date Sales`, `Year-to-Date Sales`, and `Last Year Sales` captions, and the `Column Only` sales rows including `L/Y Sales` come from `app.inventory_sales_cell`, an owned per-cell projection imported from `inventory_quantities.csv`. That restores the legacy size-grid shape without putting `rics_mirror.inventory_quantities` back on the request path.
...
The SKU Lookup modal inside the Product Inquiry page is backed by an in-memory index of the full promoted `app.sku` catalog that the API loads at startup. This is the app-owned Postgres successor to the legacy `InventoryMaster` surface. It covers every SKU in the catalog (no cap) and must stay that way. See [`docs/operations/sku-lookup-index-warmup.md`](../operations/sku-lookup-index-warmup.md) for details, verification steps, and hard rules against re-capping the index.
