...
10. **Ledger existence in legacy RICS data. RESOLVED 2026-04-19.** `RIINVHIS.MDB` contains the `InvHis` table — one row per (SKU, Store) with `AverageCost`, current `OnHand`, `LastMonthOnHand`, and rolling calendar-month arrays `LYMonthQtyOH_NN` / `LYMonthOnHand_NN` / `LYMonthQtySales_NN` / `LYMonthDolSales_NN` (NN=01..12, indexed by calendar month, rolled to the most recently completed occurrence). Alignment proven against `RITRNSSV.TicketDetail.Extension` — see `apps/api/scripts/probe-invhis-alignment.ts`. Adapter `ricsSalesHistoryByMonthAdapter.queryMonthlyInventoryHistory` consumes these columns; docs in `docs/superpowers/specs/2026-04-18-sales-history-by-month-design.md` §v2.1.
