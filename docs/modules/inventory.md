...
- **p. 70, Inventory Inquiry** — look up any SKU and see, per Store: on-hand, on-order current (At-Once) and future, model, shortage (model − on-hand), MTD / STD / YTD sales (qty), by size grid. Header shows SKU Description, Category, Vendor, Vendor SKU, Style/Color, Size Type, Last Received; top-right shows Retail, Markdown 1, Markdown 2, Avg Cost, Current Cost, List Price; middle band shows Qty / Net / Markdown / Profit for Week / Month / Season / Year. `[Prev]` / `[Next]` step through SKUs (optionally in Store-then-SKU order). Function-key display modes: F2 On Hand, F3 On Order Current, F4 On Order Future, F5 Model, F6 Short, F7 MTD Sales, F8 STD Sales, F9 YTD Sales, F11 Single Column, F12 All Stores On Hand, Shift+F1 All Stores 1 Row, Shift+F2 All Stores Summary, Shift+F3 Max, Shift+F4 Reorder. Also: `[UPCs]` (all UPCs for the SKU), `[POs]` (outstanding POs), `[Trend]` (Eight-Week Trending — see `sales-reporting`), `[Info]` (Season, Label Code, Group Code, Date 1st Received, Date Last Markdown, Perks, Comments, last-12-months sales qty + $, and the SKU/Store G.P. %, ROI, TURNS at MTD / STD / YTD), `[Detail]` (→ Inventory Change Detail).
- **p. 53 (Ch. 2) & p. 70 (Ch. 4), Inventory Inquiry (same screen, two menu entries)** — the sales-clerk register workflow (Ch. 2 p. 53) and the admin workflow (Ch. 4 p. 70) both drop into the same inquiry. Inventory owns the read-side.
- **p. 55 (Ch. 2) & p. 70 (Ch. 4), Inventory Change Detail** — per-SKU chronological log of every change: Manual Receipts, Returns, PO Receipts, Transfers In, Transfers Out, Physical Inventory Adjustments. Columns: Store #, Date, Type, Quantity, Cost, Comments (bill-to/ship-from store, PO#, RMA#). Size-detail toggle; Show All Stores toggle. Most recent first.
...
- **Inventory Inquiry** (`/inventory/skus/:skuCode`) — header (SKU + pricing + sales rollups) + view selector (On Hand / On Order Current / On Order Future / Model / Short / MTD / STD / YTD / Max / Reorder / All-Stores / Combined) + size grid + action buttons → UPCs, POs, Trend, Info, Detail (opens Change Detail panel), Prev/Next navigation.
- **Inventory Change Detail** — panel or route `/inventory/skus/:skuCode/changes`. Chronological table, size-detail toggle, store filter, date range, movement-type filter, CSV export.
