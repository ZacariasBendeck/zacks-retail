...
- **Current development-stage authority for inquiry sales totals (verified against live RICS on April 25-26, 2026)** — the Inquiry sales roll-up strip, All Stores Summary, single-row all-stores totals for single-row size types, and the `[Info]` popup's last-12-month sales + GP / ROI / Turns are read from the imported inquiry-history tables `app.inventory_history_snapshot`, `app.inventory_history_month`, and `app.inventory_history_trend_week`. The per-size MTD / STD / YTD / L/Y sales cells still replay completed tickets because no cell-level legacy inquiry-history table has been promoted yet. See [`docs/dev/specs/2026-04-26-inventory-inquiry-sales-authority.md`](../../dev/specs/2026-04-26-inventory-inquiry-sales-authority.md).
...
- **Inquiry sales totals mirror legacy inquiry-history counters, not live ticket rollups.** For Product Inquiry parity, `inventory` treats the imported `app.inventory_history_snapshot` / `app.inventory_history_month` tables as the authority for the roll-up strip, All Stores Summary, single-row all-stores totals for single-row size types, and the `[Info]` popup metrics. `L/Y Sales` is reconstructed from the visible prior-calendar-year months in `app.inventory_history_month` plus the `ly_year_qty_sales` carry field so months that have rolled out of the 12-slot history stay visible, matching the legacy RICS screen. Ticket replay remains only a temporary authority for the per-size sales grids.
