...
- **Current development-stage authority for inquiry sales totals (verified against live RICS on April 25-26, 2026)** — the Inquiry sales roll-up strip, All Stores Summary, single-row all-stores totals for single-row size types, and the `[Info]` popup's last-12-month sales + GP / ROI / Turns are read from the imported inquiry-history tables `app.inventory_history_snapshot`, `app.inventory_history_month`, and `app.inventory_history_trend_week`. The size-grid MTD / STD / YTD / L/Y cells, the backend-computed `Total` row appended to the MTD / STD / YTD / L/Y tabs, the grid-level totals shown beside the `On Hand`, `Short Quantities`, `Month-to-Date Sales`, `Season-to-Date Sales`, `Year-to-Date Sales`, and `Last Year Sales` captions, and the `Column Only` sales rows including `L/Y Sales` are read from `app.inventory_sales_cell`, the owned per-cell projection imported from `inventory_quantities.csv`. See [`docs/dev/specs/2026-04-26-inventory-inquiry-sales-authority.md`](../../dev/specs/2026-04-26-inventory-inquiry-sales-authority.md).
...
- **Inquiry sales totals mirror legacy inquiry-history counters, not live ticket rollups.** For Product Inquiry parity, `inventory` treats the imported `app.inventory_history_snapshot` / `app.inventory_history_month` tables as the authority for the roll-up strip, All Stores Summary, single-row all-stores totals for single-row size types, and the `[Info]` popup metrics. The size-grid MTD / STD / YTD / L/Y cells, the MTD / STD / YTD totals row, and the `Column Only` sales rows including `L/Y Sales` come from `app.inventory_sales_cell`, the owned projection of the legacy `inventory_quantities.csv` sales counters. `L/Y Sales` in the summary surfaces is still reconstructed from the visible prior-calendar-year months in `app.inventory_history_month` plus the `ly_year_qty_sales` carry field so months that have rolled out of the 12-slot history stay visible, matching the legacy RICS screen.
...
- `GET /api/v1/inventory/by-sku/:skuId` — full Inventory Inquiry payload: `{ header, pricing, sales: { week, month, season, year }, grids: { onHand, onOrderCurrent, onOrderFuture, model, short, mtdSales, stdSales, ytdSales, max, reorder } }` for one (SKU × Store). `?mode=allStoresOneRow | allStoresSummary` swaps the payload shape for RICS's Shift+F1 / Shift+F2 variants. In the current request path, the roll-up strip and all-store totals are backed by `app.inventory_history_snapshot` / `app.inventory_history_month`; only the per-size sales cells remain ticket-derived.
- `GET /api/v1/inventory/by-sku/:skuId/additional-info` — p. 71 `[Info]`: Season / Label Code / Group Code / Date 1st Received / Date Last Markdown / Perks / Comments + last-12-months qty + $ + MTD/STD/YTD GP% / ROI / Turns. Current request-path authority is `app.inventory_history_snapshot` + `app.inventory_history_month`.
...
10. **Ledger existence in legacy RICS data. RESOLVED 2026-04-19; close semantics refined 2026-05-04.** `RIINVHIS.MDB` contains the `InvHis` table — one row per (SKU, Store) with `AverageCost`, current `OnHand`, `LastMonthOnHand`, and rolling calendar-month arrays `LYMonthQtyOH_NN` / `LYMonthOnHand_NN` / `LYMonthQtySales_NN` / `LYMonthDolSales_NN` (NN=01..12, indexed by calendar month, rolled to the most recently completed occurrence). A May 1 before/after close comparison found that the newly closed quantity slot follows pre-close `LastMonthOnHand`, while `LastMonthOnHand` advances to current `OnHand` and the new value slot follows ending `OnHand x AverageCost` in sampled rows. Alignment proven against `RITRNSSV.TicketDetail.Extension` — see `apps/api/scripts/rics/probes/probe-invhis-alignment.ts`. Adapter `ricsSalesHistoryByMonthAdapter.queryMonthlyInventoryHistory` consumes these columns; docs in `docs/dev/specs/2026-04-18-sales-history-by-month-design.md` §v2.1.
