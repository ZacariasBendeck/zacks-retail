...
- **Eight Week Trending period "reset" semantics preserved, but made independent of Close Week.** RICS says the trending is inaccurate unless Close Week runs every week on the same day (p. 91). That ties the analytics to a manual operator step. Zack's Retail anchors "week" to a configurable `fiscal_week_ends_on` setting in `store-ops` Company Setup and automatically partitions events against that anchor — no manual close needed for trending to work. (The Close Week operation stays in `accounts-receivable` for other fiscal-period reasons.)
