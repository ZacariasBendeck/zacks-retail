...
Vercel is not the place to handle MDB files, and hosted Postgres should not carry a raw legacy mirror schema.
...
4. verify owned-table counts and operator workflows before Vercel promotion.
...
3. **Promotion phase**:
   - after the data-side validation passes, promote the Vercel production deployment and run smoke tests.
...
7. rehearse the Vercel promotion and rollback steps.
