...
2. **No new branches, no worktrees.** Ever. All work is committed directly to `master`. Reconfirmed this session — stays in effect.
...
4. **Ignore WAT vocabulary.** The grandparent `CompartidoZBIA/CLAUDE.md` describes an older WAT framework; that does not apply here. Don't reintroduce WAT vocabulary.
...
3. **Write the Postgres architecture design spec** at `docs/dev/specs/2026-04-20-postgres-architecture-design.md`. Present sections, get per-section approval, commit, self-review. Content is already 90% thought-through in section 5 above; you're formalizing, not inventing.
4. **Get operator approval on the spec.**
5. **Write a Phase 0 implementation plan** at `docs/dev/plans/` (stand up Postgres instance + baseline shared schema + `platform.audit_log` + `platform.etl_run` + empty module schemas + ETL runner skeleton). Phase 0 is infrastructure only — no user-visible change.
6. **Execute Phase 0.**
