...
- **Legacy RICS MDB files** in `E:/data/rics-mdbs/` are **read-only and never written to**. They are touched by exactly one process: the operator-invoked `pnpm sync:rics` ETL, which copies them into Postgres. Request handlers never open an MDB at request time.
- **Postgres `rics_mirror` schema** holds a 1:1 mirror of every canonical RICS table, rebuilt atomically on each `sync:rics` invocation. It is imported RICS source data for ETL/bootstrap and for live reads only until that surface has an app-owned authoritative table. Once a surface is authoritative in `app.*` or a module-owned Postgres schema, request handlers read only from that app-owned surface; `rics_mirror` is no longer legal in the live request path for it. See [docs/operations/rics-mirror-sync.md](docs/operations/rics-mirror-sync.md).
- **Postgres `public` + `app` schemas** hold net-new Zack's Retail data such as content overlays, cart, orders, auth, drafts, and configuration. These are **preserved across reloads** - the ETL only touches `rics_mirror`.
- **Postgres `platform` schema** holds the cross-cutting admin spine: `etl_run`, `etl_run_table` (sync audit log). Future platform tables are listed in [docs/modules/platform.md](docs/modules/platform.md).
...
The OLEDB adapter ([`apps/api/src/services/accessOleDb.ts`](apps/api/src/services/accessOleDb.ts), [`ricsProductAdapter.ts`](apps/api/src/services/ricsProductAdapter.ts)) stays in the codebase during **Development Against RICS Mirror** because the sync ETL still uses it for column introspection. The per-request read paths that still use it (listed in [docs/operations/rics-mirror-sync.md](docs/operations/rics-mirror-sync.md)) migrate first to `rics_mirror` reads module by module, then off `rics_mirror` entirely once the module has an app-owned authoritative table. The `PRODUCT_SOURCE=rics|local` flag is legacy from the prior design and is no longer the cutover mechanism.
...
The project follows three rollout stages. Always know which stage a piece of work belongs to - it determines what data source is legal and which audiences are affected by a regression. The plan shifted in 2026-04 to a one-way mirror approach with no bidirectional sync and no writes back to RICS.

**Development Against RICS Mirror.** RICS remains the live production system until cutover day. An operator-invoked ETL ([docs/operations/rics-mirror-sync.md](docs/operations/rics-mirror-sync.md)) reloads every canonical RICS table from the MDB files into the Postgres `rics_mirror` schema. During this stage:
...
- The final extraction/import runs
- Mirror data plus app-owned extensions are promoted into module-owned schemas
...
- The C# bulk extractor, the `bulk-extract.ps1` host, the `accessOleDb.ts` and `persistentPwsh.ts` helpers, and the `rics_mirror` schema itself are removed
...
- During **Development Against RICS Mirror**, a feature may read from `rics_mirror.*` only while that surface has no app-owned authoritative table yet; once such a table exists, request handlers read only from the app-owned surface.
- Development-stage writes land in `public.*` or `app.*`.
- Features must never write into `rics_mirror` and must never write back to the MDBs.
...
- App-native improvements such as new columns, richer content, and audit trails live in `public` or `app` schemas alongside the mirror, never inside `rics_mirror`.
...
1. **Slash commands in [`.claude/commands/`](.claude/commands/)** - project-specific rituals tailored to this repo's paths and conventions. Invoke via `/<name>`. Current examples: `/sync-module-docs`, `/new-manual-chapter`, `/new-module-spec`, `/verify-rics-mirror`.
...
- ETL: PowerShell + C# (hosted via `Add-Type`) reads ACE.OLEDB.12.0, writes CSV, and Node pipes into Postgres `COPY`. One-way RICS -> Postgres, operator-invoked. See [docs/operations/rics-mirror-sync.md](docs/operations/rics-mirror-sync.md).
...
- **New lookup data** such as families, brands, stores, employees, and promotion codes lives in `app.*` or `rics_mirror.*`, never in a fresh SQLite table.
