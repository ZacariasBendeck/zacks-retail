...
This repo uses slash commands under [`.Codex/commands/`](.Codex/commands/) as the primary workflow surface. Subagents are not used.
...
1. **Slash commands in [`.Codex/commands/`](.Codex/commands/)** — project-specific rituals tailored to this repo's paths and conventions. Invoke via `/<name>`. Current: `/sync-module-docs`, `/new-manual-chapter`, `/new-module-spec`, `/verify-rics-mirror` (see that folder for the full set). These beat generic marketplace agents for this codebase.
2. **Skills** — content bundles whose rule files can be referenced directly. Example: the Supabase Postgres best-practices skill at `E:/dev/.Codex/skills/supabase-postgres-best-practices/`.
3. **Plain Codex** — for architectural, cross-module, or scope-unclear work.

**Rule of thumb:** project-specific workflow → write a slash command. One-off investigation → plain Codex. Reusable domain rules → install a skill.
