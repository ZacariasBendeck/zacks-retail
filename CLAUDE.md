...
- **Postgres `rics_mirror` schema** holds a 1:1 mirror of every canonical RICS table, rebuilt atomically on each `sync:rics` invocation. It is the imported RICS source for ETL/bootstrap and for live reads only until that surface has an app-owned authoritative table. Once a surface is authoritative in `app.*` / module-owned Postgres tables, request handlers read only from that app-owned surface; `rics_mirror` is no longer legal in the live request path for it (see [docs/operations/rics-mirror-sync.md](docs/operations/rics-mirror-sync.md)).
...
**Phase A — Mirror-backed dev against live RICS.** RICS stays live in the stores; store operators keep entering sales and POs into it as usual. Separately, an operator-invoked ETL ([docs/operations/rics-mirror-sync.md](docs/operations/rics-mirror-sync.md)) reloads every canonical RICS table from the MDB files into the Postgres `rics_mirror` schema in ~5 min. Until a surface has an app-owned authoritative table, Zack's Retail may read that surface from `rics_mirror` (never from the MDBs at request time); writes land in `public`/`app` Postgres tables that the reload explicitly does not touch. Once a surface has an app-owned authoritative table, request handlers read only from that app-owned table and `rics_mirror` becomes ETL/bootstrap input only for that surface. No reverse sync — Zack's Retail and RICS are independent surfaces during this phase.
...
- A new feature's spec must declare which phase it targets. A "Phase A" feature may read from `rics_mirror.*` only while that surface has no app-owned authoritative table yet; once such a table exists, request handlers read only from the app-owned surface. Phase-A writes land in `public.*` or `app.*`. Features MUST NOT write into `rics_mirror` (reload drops everything) and MUST NOT write back to the MDBs (hard rule; see below).
...
- Module specs in `docs/modules/<name>.md` should record the current request-path authority for each surface. Once a module owns an authoritative `app.*` / module table for a surface, the spec should say that live reads come only from that table and that `rics_mirror` is bootstrap/ETL-only for it.
