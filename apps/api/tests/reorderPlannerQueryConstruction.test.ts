...
describe('reorder planner sales query construction', () => {
  ...
  it('loads monthly SKU sales through sku_id without normalized sku_code filters', async () => {
    ...
    expect(sql).toContain('WHERE l.sku_id = $1::uuid');
  ...
  it('uses an exact sku_code fallback only when sku_id sales are absent', async () => {
    ...
    expect(idSql).toContain('WHERE l.sku_id = $1::uuid');
    expect(fallbackSql).toContain('WHERE l.sku_code = $1');
