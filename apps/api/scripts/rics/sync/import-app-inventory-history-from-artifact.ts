...
function parseArgs(): Args {
  const args: Args = { manifestPath: null, asOf: null };
...
function printHelpAndExit(code: number): never {
  console.log(
    [
      'Usage: import-app-inventory-history-from-artifact --manifest <path> [--as-of YYYY-MM-DD]',
      '',
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `[import:app-inventory-history-from-artifact] staged ${invHisTable.targetTable} ` +
    ...
    console.log(
      `[import:app-inventory-history-from-artifact] OK - snapshots=${fmtNum(result.importedSnapshots)} ` +
    ...
    if (result.unresolvedSkuRows > 0) {
      console.warn(
        `[import:app-inventory-history-from-artifact] unresolved sku links=${fmtNum(result.unresolvedSkuRows)} ` +
...
main().catch((err) => {
  console.error(`[import:app-inventory-history-from-artifact] FAILED - ${err?.message ?? err}`);
