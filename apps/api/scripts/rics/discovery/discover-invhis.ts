/**
 * One-shot discovery pass for RIINVHIS.MDB — inventory history.
 *
 * Dumps tables + columns + a handful of sample rows so we can decide whether
...
const FILES = ['RIINVHIS.MDB'];

function truncate(v: unknown, n = 160): string {
...
async function main() {
  for (const file of FILES) {
    ...
    for (const table of tables) {
      ...
      // Row count
      try {
        const cntRaw = runPowerShellJson<{ N: number } | { N: number }[]>(
          buildSelectScript(dbPath, password, `SELECT COUNT(*) AS N FROM [${table}]`),
        );
        const cnt = Array.isArray(cntRaw) ? cntRaw[0] : cntRaw;
      ...
      try {
        const rowsRaw = runPowerShellJson<Record<string, unknown> | Record<string, unknown>[]>(
          buildSelectScript(dbPath, password, `SELECT TOP 3 * FROM [${table}]`),
        );
